...
mod tests {
    ...
    async fn get_existing() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = get(&store, Bytes::from_static(b"k")).await.unwrap();
    ...
    async fn set_returns_ok_and_mutation() {
        let store = spawn_test_store();
        let frame = set(&store, Bytes::from_static(b"k"), entry(b"v", Expiry::None))
            .await
            .unwrap();
    ...
    async fn getdel_existing() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = getdel(&store, Bytes::from_static(b"k")).await.unwrap();
        assert_eq!(frame, Frame::BulkString("v".into()));
        assert!(store.get(Bytes::from_static(b"k")).await.unwrap().is_none());
    }
    ...
    async fn setnx_on_missing() {
        let store = spawn_test_store();
        let frame = setnx(&store, Bytes::from_static(b"k"), entry(b"v", Expiry::None))
            .await
            .unwrap();
    ...
    async fn incr_existing() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = incr(&store, Bytes::from_static(b"k")).await.unwrap();
    ...
    async fn incr_non_integer_errors() {
        ...
        store
            ...
            .await
            .unwrap();
        let result = incr(&store, Bytes::from_static(b"k")).await;
    ...
    async fn decr_existing() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = decr(&store, Bytes::from_static(b"k")).await.unwrap();
    ...
    async fn strlen_existing() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = strlen(&store, Bytes::from_static(b"k")).await.unwrap();
    ...
    async fn append_new_key() {
        let store = spawn_test_store();
        let frame = append(&store, Bytes::from_static(b"k"), Bytes::from_static(b"abc"))
            .await
            .unwrap();
    ...
    async fn ttl_missing() {
        let store = spawn_test_store();
        let frame = ttl(&store, Bytes::from_static(b"k"), 0).await.unwrap();
    ...
    async fn ttl_with_expiry() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = ttl(&store, Bytes::from_static(b"k"), now).await.unwrap();
    ...
    async fn pttl_missing() {
        let store = spawn_test_store();
        let frame = pttl(&store, Bytes::from_static(b"k"), 0).await.unwrap();
        assert_eq!(frame, Frame::Integer(-2));
    }
    ...
    async fn pttl_with_expiry() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = pttl(&store, Bytes::from_static(b"k"), now).await.unwrap();
    ...
    async fn persist_existing_with_expiry() {
        ...
        store
            ...
            .await
            .unwrap();
        let frame = persist(&store, Bytes::from_static(b"k")).await.unwrap();
        assert_eq!(frame, Frame::Integer(1));
    }
    ...
    async fn expire_existing() {
        ...
        store
            ...
            .unwrap();
        let now = crate::utils::time::get_current_millis();
        let frame = expire(&store, Bytes::from_static(b"k"), 5000, now)
            .await
            .unwrap();
        assert_eq!(frame, Frame::Integer(1));
    }
